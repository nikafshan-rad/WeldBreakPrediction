...
f = open("inbalanced-models.txt","w")
for i in range(0,len(clfs)):
    for j in range(0,len(samplers)):
        Xtrain1, Ytrain1 = samplers[j].fit_sample(Xtrain,Ytrain)
        y_pred = clfs[i].fit(Xtrain1, Ytrain1).predict(Xtest)    
        f.write("########################################" +
                clfs_name[i] + " with SamplereMethod = " + 
                samplers_name[j] + 
                " ########################################" + '\n')
